...
import TradingViewWidgets from 'react-tradingview-widget';
import ChatBot from './ChatBot';
import LangConverter from './LangConverter';
...
async function fetchSummary(symbol) {
  try {
    const res = await axios.get(`http://localhost:5000/api/strategy?ticker=${symbol}`);
    return res.data;
  } catch (err) {
    console.error(err);
    return '';
...
const StockCard = ({ ChartText, symbol }) => {
  ...
  return (
    <div>
      ...
      >
        {isFullView ? (
          <div style={{ position: 'relative', top: '80px', right: '10px' }}>
            <TradingViewWidgets
              symbol={String(symbol).replace('.NS', '')}
              theme="light"
              interval="1"
              ...
              hotlist={true}
              calendar={true}
              container_id="tradingview-zomato"
              height="650"
              width="1500"
            />
          </div>
                      ...
                      <div className="news-item" key={index}>
                        ...
                        {lang !== 'en' ? (
                          <span className="news-title">
                            <LangConverter inputText={item} lang={lang} />
                          </span>
                        ) : (
